					...
					if (!stdout1) {
						...
						exec(getPathInfo(urlLocal).drive + ": && cd " + getPathInfo(urlLocal).directory + " && svn merge --accept mc " + urlBranches, function(error2, stdout2, stderr2) {
							...
							if (conflictTag) {
								...
								commit(urlLocal, suffixTrunk, revisions);
								buildTask(urlTrunk);
							}
						})
					}
...
* 提交代码处理
*/
function commit(urlLocal, suffixTrunk, revisions) {
	console.log("\n开始执行commit操作......");
	exec(getPathInfo(urlLocal).drive + ": && cd " + getPathInfo(urlLocal).directory + " && svn commit " + urlLocal + " -m 'merge  " + revisions + "  into trunk from " + suffixTrunk + "'", function(error, stdout, stderr) {
		...
		console.log(suffixTrunk + "合并操作执行成功！");
	})
...
function buildStaticProject() {
	console.log(getPathInfo(JENKINS_PATH).drive, getPathInfo(JENKINS_PATH).directory)
	try	{
